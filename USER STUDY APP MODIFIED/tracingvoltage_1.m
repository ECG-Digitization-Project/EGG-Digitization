function [app] = tracingvoltage_1(app)
...
%         pause; 
        ...
        first_non_zero = find(signal,1);
        last_non_zero = length(signal) - find(fliplr(signal),1) + 1;
        
       
        x = signal(first_non_zero:last_non_zero);
        nanx = isnan(x);
        t    = 1:numel(x);
        x(nanx) = interp1(t(~nanx), x(~nanx), t(nanx));
        signal(first_non_zero:last_non_zero) = x; 
